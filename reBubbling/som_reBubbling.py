...
if __name__ == "__main__":
	...
	# put in the dish 950ml of water and a oil droplet
	...
	cv.Flip(frame, frame, 1) # flip around x because the webcam mirrored

	# to store the results from the color seg
	colorThreshed = cv.CreateImage(cv.GetSize(frame), 8, 1)

	key = 1 # key is keyboard input
	FPS = cv.GetCaptureProperty(capture, cv.CV_CAP_PROP_FPS) #in the case of grabbing from a webcam this returns 0
	FPS = 20 # somehow the previous line returns 0 for cams. 30 is the default. 20 cos my computer is slow
	frame_period = 1.0 / FPS

	cv.SetMouseCallback('video', mouseHandler, frame)

	droplets = [] # this will hold all the droplets found
	frames = 0 # to count the number of frames and do something every 'x' frames

	rebubbling = 0 #when this is set to 1, an oil droplet will be rebubbled
	...
	while(1):
		...
		frame  = cv.QueryFrame(capture) # grab 1 new frame
		if not frame: break
		t = cv.CloneImage(frame)
		cv.Remap( t, frame, mapx, mapy ) #undistort
		...
		frames += 1
		# we will do something every 1s if we checked frames == 0
		if frames > FPS: 
			frames = 0

		foundDrops = 0 # this will hold whatever is returned by the connected comp analysis
		if frames == 0 and len(droplets) > 0:
			...
			for current in droplets:
				...
				cv.InRangeS(frame, minRange, maxRange, colorThreshedTemp) #this performs the color segmentation
			...
			if foundDrops > 0 and foundDrops == len(droplets):
				...
				if (rebubbling == 10): #when it's been 10 iterations in a row without moving, let's insert more oil on it
					...
					gcode  += [command, 'G4 P500', 'M43 P3 S169', 'G4 P500', 'M43 P2 S55', 'G4 P500', 'M43 P3 S180']
					gcode +=  ['G4 P500', 'M43 P2 S0', 'G4 P500', 'G1 X183 Y92', 'G4 P500']
					sleep(3)
					p.startprint(gcode)
					sleep(3)
					rebubbling = 0
				...
				for i in xrange(len(droplets)):
					...
					changeDirection = direction - droplets[i]['direction']
					...
					pos = som.FindBestMatchingNode([ float(speed) / som.dimMaxs[0], float(changeDirection)/som.dimMaxs[1]])
					...
					if pos[0] < 2 and pos[1] < 5: # in this conditions we consider it's not moving
						rebubbling += 1
						print rebubbling
					else:
						rebubbling = 0 #it moved or shaked, reset the counter
